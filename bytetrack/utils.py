...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None, classes=None, score=None, clsnames=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        cv.putText(im, '%s %.1f' % (clsnames[int(classes[i])], score[i] * 100), 
        (intbox[0]+15, intbox[1]), cv.FONT_HERSHEY_PLAIN, text_scale, color, thickness = text_thickness, lineType=cv.LINE_AA)
